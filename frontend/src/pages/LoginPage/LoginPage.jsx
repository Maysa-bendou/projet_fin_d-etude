...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await fetch("https://ticket-backend-4uw2.onrender.com/api/auth/login", {
        method: "POST",
    ...
    جازی
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div>
           ...
           <div className="mt-1 flex items-center gap-3 px-4 py-2.5 bg-[#faf9f7] border border-[#e8e4df] rounded-lg focus-within:ring-2 focus-within:ring-red-200">   <Lock size={16} className="text-slate-400" />
              <input
                type="password"
                required
                value={password}
