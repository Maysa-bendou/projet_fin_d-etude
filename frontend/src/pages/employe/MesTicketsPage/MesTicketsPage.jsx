...
import { formatDate, translateKey, STATUS_KEYS, PRIORITY_KEYS, CATEGORY_KEYS } from "../../../constants/ticketKeys";
...
export default function MesTicketsPage() {
 const { t, i18n } = useTranslation(["employee", "common"]);
  ...
  // i18n-aware date formatter — uses the locale from common.json → date.locale
  const fmtDate = (str) => formatDate(t, str, "long");

  // Safe BCP-47 locale for direct Intl calls (month dropdown)
  const VALID_LOCALE_RE = /^[a-zA-Z]{2,3}(-[a-zA-Z0-9]{2,8})*$/;
  const safeLocale = useMemo(() => {
    const tag = t("date.locale", { ns: "common" });
    return tag && VALID_LOCALE_RE.test(tag) ? tag : (navigator.language || "fr-FR");
  ...
  const filteredActuels  = useMemo(() => filterList(actuelsList,  false), [actuelsList,  filterSearch, filterStatus, filterPriority, filterService, filterCategory, filterMonth,             currentLang]); // eslint-disable-line
  const filteredArchives = useMemo(() => filterList(archivesList, true),  [archivesList, filterSearch, filterStatus, filterPriority, filterService, filterCategory, filterMonth, filterYear, currentLang]); // eslint-disable-line
  ...
  if (loading) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: "#fbfbfb" }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 2 }}>{t("loading")}</span>
      </div>
    </div>
  );
    ...
    >
      ...
      <div style={{ padding: "20px 28px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 14, background: "#fff", border: "1px solid #e8e2d9", borderRadius: 10, padding: "7px 12px", overflowX: "auto" }}>
          ...
          <FilterInput
            placeholder={t("mesTickets.filters.search")}
            value={filterSearch}
          ...
          <FilterSelect icon={HiOutlineTag} value={filterCategory} onChange={setFilterCategory} minW={110}>
            ...
            {dbEnums.categories?.map(c => <option key={c} value={c}>{translateKey(t, CATEGORY_KEYS, c)}</option>)}
          </FilterSelect>
          <FilterSelect icon={HiOutlineExclamationCircle} value={filterStatus} onChange={setFilterStatus} minW={105}>
            ...
            {dbEnums.statuts?.map(s => <option key={s} value={s}>{translateKey(t, STATUS_KEYS, s)}</option>)}
          </FilterSelect>
          <FilterSelect icon={HiOutlineFunnel} value={filterPriority} onChange={setFilterPriority} minW={110}>
            ...
            {dbEnums.priorites?.map(p => <option key={p} value={p}>{translateKey(t, PRIORITY_KEYS, p)}</option>)}
          </FilterSelect>
...
<FilterSelect key={currentLang} icon={HiOutlineCalendarDays} value={filterMonth} onChange={setFilterMonth} minW={90}>
  <option value="">{t("mesTickets.filters.mois")}</option>
  {Array.from({ length: 12 }, (_, i) => (
    <option key={i} value={i + 1}>
      {new Date(2000, i).toLocaleString(safeLocale, { month: "short" })}
    </option>
  ))}
</FilterSelect>
