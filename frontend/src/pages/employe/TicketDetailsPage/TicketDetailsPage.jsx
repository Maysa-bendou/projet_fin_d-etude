...
import {
  ...
  HiOutlineCalendarDays, HiOutlineChevronDown, HiOutlineChevronUp,
  HiOutlineHashtag, HiOutlineShieldCheck, HiOutlineBeaker,
  ...
  return (
    <div style={{ background:"#fff", borderRadius:16, border:"1px solid #e2e8f0", overflow:"hidden" }}>
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        <HiOutlineClock size={14} color="#94a3b8" />
        ...
        {open
          ? <HiOutlineChevronUp size={14} color="#94a3b8" />
  ...
  return (
    <div style={{ borderTop:"1px solid #f1f5f9" }}>
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        <HiOutlineCheckCircle size={14} color="#64748b" />
        <span style={{ fontSize:12, fontWeight:700, color:"#15803d", flex:1 }}>{t('ticketDetails.solution.title')}</span>
        {open
          ? <HiOutlineChevronUp size={13} color="#94a3b8" />
          : <HiOutlineChevronDown size={13} color="#94a3b8" />}
      </button>
      {open && (
        <div style={{ padding:"0 22px 16px", borderTop:"1px solid #f1f5f9" }}>
...
export default function TicketDetailsPage() {
 ...
 useEffect(() => {
  ...
  if (loading) return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:280, gap:10, background:"#f8fafc" }}>
  ...
  if (error) return <div style={{ color:"#dc2626", textAlign:"center", padding:48, background:"#f8fafc", minHeight:"100vh" }}>{error}</div>;
  ...
  const btnOutline = { background:"#fff", border:"1px solid #e2e8f0", borderRadius:8, padding:"7px 12px", fontSize:12, fontWeight:500, color:"#374151", cursor:"pointer", display:"flex", alignItems:"center", gap:5 };
  const inputStyle = { width:"100%", padding:"8px 12px", border:"1px solid #e2e8f0", borderRadius:8, fontSize:13, color:"#1e293b", outline:"none", fontFamily:"inherit", background:"#fff" };
  ...
  // Spec cells — inline layout: Icon  Label : value
  const specCells = [
    { icon: HiOutlineBolt,                label: t('ticketDetails.details.priority'),
      node: <Pill config={PRIORITY_CONFIG} value={ticket.priority} label={translateKey(t, PRIORITY_KEYS, ticket.priority)} /> },
    { icon: HiOutlineTag,                 label: t('ticketDetails.details.category'),
      node: <Pill config={CATEGORY_CONFIG} value={ticket.category} label={translateKey(t, CATEGORY_KEYS, ticket.category)} /> },
    { icon: HiOutlineWrenchScrewdriver,   label: t('ticketDetails.details.itService'),
      node: <span style={{ fontSize:12, fontWeight:600, color:"#0f172a" }}>{ticket.service ?? "N/A"}</span> },
    { icon: HiOutlineExclamationTriangle, label: t('ticketDetails.details.impact'),
      node: isEditing
        ? <select value={editFields.impact} onChange={e => handleFieldChange("impact", e.target.value)}
            style={{ border:"1px solid #e2e8f0", borderRadius:6, padding:"4px 8px", fontSize:12, outline:"none", background:"#fff" }}>
    ...
    { icon: HiOutlineCalendarDays,        label: t('ticketDetails.details.createdAt'),
      node: <span style={{ fontSize:12, fontWeight:600, color:"#0f172a" }}>{fmtDate(ticket.createdAt)}</span> },
  ];

  // Initial attachments
  const attachmentComments = comments.filter(c => c.comment_type === "attachment");
  const allAttachFiles = attachmentComments.flatMap(c => c.files ?? []);
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#faf9f7", fontFamily:"sans-serif" }}>
      ...
      {/* ── HEADER ── */}
      <div style={{ background:"#faf9f7", borderBottom:"1px solid #e2e8f0", padding:"12px 28px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <button onClick={() => navigate(-1)} style={{ display:"flex", alignItems:"center", gap:6, background:"none", border:"none", cursor:"pointer", color:"#94a3b8", fontSize:12, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.1em", padding:0 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          {isEditing ? (
            ...
            <>
              ...
              {canReopen && (
                <button onClick={handleReopen} style={{ ...btnOutline, color:"#374151" }}>
          ...
          <button onClick={() => prevId && navigate(`/employee/ticket/${prevId}`)} disabled={!prevId}
            style={{ display:"flex", alignItems:"center", gap:5, padding:"7px 14px", borderRadius:8, border:"1px solid #e2e8f0", background:"#fff", cursor:!prevId ? "not-allowed" : "pointer", color:!prevId ? "#c4bdb3" : "#374151", fontSize:12, fontWeight:700 }}>
          ...
          <button onClick={() => nextId && navigate(`/employee/ticket/${nextId}`)} disabled={!nextId}
            style={{ display:"flex", alignItems:"center", gap:5, padding:"7px 14px", borderRadius:8, border:"1px solid #e2e8f0", background:"#fff", cursor:!nextId ? "not-allowed" : "pointer", color:!nextId ? "#c4bdb3" : "#374151", fontSize:12, fontWeight:700 }}>
      ...
      <div style={{ padding:"20px 28px", display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ background:"#fff", borderRadius:16, border:"1px solid #e2e8f0", overflow:"hidden" }}>
          ...
          {/* Title + status */}
          <div style={{ padding:"16px 22px", borderBottom:"1px solid #f1f5f9", display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:16 }}>
            <div>
          ...
          {/* Specs grid — inline label layout */}
          <div style={{ padding:"14px 22px", borderBottom:"1px solid #f1f5f9" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8 }}>
              {specCells.map(({ icon: Icon, label, node }, i) => (
                <div key={i} style={{ background:"#fff", border:"1px solid #f1f5f9", borderRadius:10, padding:"10px 14px", display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                  <Icon size={13} color="#c4bfb8" style={{ flexShrink:0 }} />
                  <span style={{ fontSize:10, fontWeight:700, color:"#94a3b8", textTransform:"uppercase", letterSpacing:"0.07em", whiteSpace:"nowrap" }}>{label} :</span>
                  <div>{node}</div>
                </div>
              ))}
            </div>
          ...
          {/* Attachments — below specs */}
          {allAttachFiles.length > 0 && (
            <div style={{ padding:"12px 22px", borderBottom:"1px solid #f1f5f9", display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
              <HiOutlinePaperClip size={13} color="#94a3b8" />
          ...
          {/* Assigned tech */}
          <div style={{ padding:"12px 22px", borderBottom: hasSolution ? "1px solid #f1f5f9" : "none", display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:28, height:28, borderRadius:"50%", background:"#eff6ff", border:"1px solid #bfdbfe", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
              <span style={{ fontSize:9, fontWeight:800, color:"#1d4ed8" }}>{techInitials}</span>
            </div>
            <div>
            ...
            <div style={{ borderTop:"1px solid #f1f5f9" }}>
              {!isClosed ? (
                <>
                  ...
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"8px 18px", background:"#f8fafc", borderTop:"1px solid #f1f5f9" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
