...
const TYPE_META = {
  solution:         { label:"Solution proposée",     dot:"#1d4ed8", bg:"#eff6ff", border:"#bfdbfe", text:"#1d4ed8",   Icon: HiOutlineWrenchScrewdriver },
  info:             { label:"Information",            dot:"#d97706", bg:"#fffbeb", border:"#fde68a", text:"#92400e",   Icon: HiOutlineInformationCircle },
  confirm:          { label:"Confirmation demandée",  dot:"#15803d", bg:"#f0fdf4", border:"#bbf7d0", text:"#15803d",   Icon: HiOutlineExclamationTriangle },
  emp_reply:        { label:"Votre réponse",          dot:"#6b7280", bg:"#f9fafb", border:"#e5e7eb", text:"#374151",   Icon: HiOutlineUser },
  confirmed:        { label:"Résolution confirmée",   dot:"#15803d", bg:"#f0fdf4", border:"#bbf7d0", text:"#15803d",   Icon: HiOutlineCheckCircle },
  rejected_confirm: { label:"Solution refusée",       dot:"#dc2626", bg:"#fef2f2", border:"#fecaca", text:"#dc2626",   Icon: HiOutlineXMark },
  redirect:         { label:"Ticket redirigé",        dot:"#9333ea", bg:"#faf5ff", border:"#e9d5ff", text:"#7e22ce",   Icon: HiOutlineArrowPath },
  comment:          { label:"Note interne",           dot:"#9ca3af", bg:"#f9fafb", border:"#e5e7eb", text:"#6b7280",   Icon: HiOutlineInformationCircle },
  status:           { label:"Changement de statut",   dot:"#534ab7", bg:"#f5f3ff", border:"#ddd6fe", text:"#4c1d95",   Icon: HiOutlineBolt },
  update:           { label:"Modification",           dot:"#534ab7", bg:"#f5f3ff", border:"#ddd6fe", text:"#4c1d95",   Icon: HiOutlinePencilSquare },
  reopen:           { label:"Ticket réouvert",        dot:"#15803d", bg:"#f0fdf4", border:"#bbf7d0", text:"#15803d",   Icon: HiOutlineArrowPath },
  attachment:       { label:"Pièces jointes",         dot:"#0369a1", bg:"#f0f9ff", border:"#bae6fd", text:"#0369a1",   Icon: HiOutlinePaperClip },
};
...
export default function TicketDetailsPage() {
  ...
  if (loading) return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:280, gap:10, background:"#f9f6f2" }}>
      ...
      <span style={{ fontSize:13, color:"#94a3b8", fontWeight:500 }}>Chargement du ticket…</span>
      ...
      <div style={{ maxWidth:1020, margin:"0 auto", display:"flex", flexDirection:"column", gap:20 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:"#94a3b8" }}>
          <button onClick={() => navigate(-1)} style={{ background:"none", border:"none", cursor:"pointer", color:"#534ab7", fontSize:12, fontWeight:600, padding:0, display:"flex", alignItems:"center", gap:4 }}>
            <HiOutlineArrowLeft size={13} /> Retour
          </button>
        ...
        <div style={{ ...card, padding:"16px 20px" }}>
          <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12, flexWrap:"wrap" }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              {isEditing ? (
                <>
                  <button onClick={() => setIsEditing(false)} style={btnOutline}><HiOutlineXMark size={13} /> Annuler</button>
                  <button onClick={handleUpdateTicket} disabled={updating} style={{ ...btnPrimary, opacity:updating?.6:1 }}>
                    <HiOutlineCheck size={13} /> {updating ? "Enregistrement…" : "Enregistrer"}
                  </button>
                </>
              ) : (
                <>
                  {!isClosed && (
                    <button onClick={() => setIsEditing(true)} style={btnOutline}><HiOutlinePencilSquare size={13} /> Modifier</button>
                  )}
                  {canReopen && (
    ...
    {reopenCount >= 2 ? "Limite réouvertures atteinte" : "Délai dépassé"}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"250px minmax(0,1fr)", gap:16, alignItems:"start" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            {/* technicien */}
            <div style={card}>
              <span style={sectionLabel}>Technicien assigné</span>
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize:11, color:"#94a3b8", margin:0 }}>Technicien IT</p>
                </div>
              </div>
            </div>
        ...
        <p style={{ fontSize:12, color:"#166534", margin:0 }}>{ticket.closing_note}</p>
            ...
            <div style={card}>
              ...
              <div>
                ...
                <MetaRow label="Impact" icon={HiOutlineUser}>
                  {isEditing ? (
                    <select value={editFields.impact} onChange={e => handleFieldChange("impact", e.target.value)}
                      ...
                      <option value="low">Une personne</option>
                ...
                <MetaRow label="Urgence" icon={HiOutlineBolt}>
                  {isEditing ? (
                    <select value={editFields.urgence} onChange={e => handleFieldChange("urgence", e.target.value)}
                      style={{ border:"1px solid #d9d4cc", borderRadius:6, padding:"3px 6px", fontSize:12, outline:"none", background:"#fff" }}>
                      <option value="low">Peut travailler</option>
                ...
                <MetaRow label="Type" icon={HiOutlineInformationCircle}>{ticket.type ?? "N/A"}</MetaRow>
                <MetaRow label="Créé le" icon={HiOutlineCalendarDays}>
                  {ticket.createdAt ? new Date(ticket.createdAt).toLocaleDateString("fr-DZ") : "N/A"}
                </MetaRow>
                <MetaRow label="Mis à jour" icon={HiOutlineClock}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            ...
            <div style={card}>
              <span style={sectionLabel}>Historique des activités</span>
              {comments.filter(c => c.comment_type !== "attachment").length === 0 ? (
                <p style={{ fontSize:13, color:"#94a3b8", margin:0, padding:"8px 0" }}>Aucune activité enregistrée.</p>
              ) : (
                    ...
                    return (
                      <div key={c.id} style={{ position:"relative", marginBottom: i === arr.length-1 ? 0 : 12 }}>
                        ...
                        <div style={{ padding:"9px 12px", borderRadius:8, background:meta.bg, border:`1px solid ${meta.border}` }}>
                          ...
                          {c.author && <p style={{ fontSize:10, color:"#94a3b8", marginTop:4, marginBottom:0 }}>par {c.author}</p>}
                        </div>
                      </div>
                    );
                  })}
            ...
            {/* ── CONVERSATION ── */}
            <div style={card}>
              <span style={sectionLabel}>Échanges avec le technicien</span>
              ...
              <div style={{ maxHeight:360, overflowY:"auto", paddingRight:2, marginBottom:14 }}>
                {comments.filter(c => !["status","update","reopen","redirect"].includes(c.comment_type)).length === 0 ? (
                  <p style={{ textAlign:"center", color:"#94a3b8", padding:"32px 0", fontSize:13 }}>Aucun échange pour le moment.</p>
                ) : (
                  comments
                        ...
                        return (
                          <div key={c.id} style={{ marginBottom:10, padding:"8px 12px", background:"#f0f9ff", border:"1px solid #bae6fd", borderRadius:8 }}>
                            <div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:6 }}>
                              ...
                              <span style={{ fontSize:11, fontWeight:600, color:"#0369a1" }}>Pièces jointes initiales</span>
                            </div>
              ...
              {pendingConfirm && (
                <div style={{ background:"#f0fdf4", border:"1px solid #bbf7d0", borderRadius:10, padding:"14px 16px", marginBottom:12 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:10 }}>
                    ...
                    <p style={{ fontSize:13, fontWeight:600, color:"#15803d", margin:0 }}>Cette solution a-t-elle résolu votre problème ?</p>
                  </div>
                  <div style={{ display:"flex", gap:8 }}>
                    <button onClick={() => handleConfirmReply(true)} disabled={confirming}
                      ...
                      <HiOutlineCheck size={13}/> Oui, résolu
                    </button>
                    <button onClick={() => handleConfirmReply(false)} disabled={confirming}
                      ...
                      <HiOutlineXMark size={13}/> Non, persiste
                    </button>
                  </div>
                </div>
              )}
              ...
              {alreadyConfirmed && (
                <div style={{ background:"#f9fafb", border:"1px solid #e5e7eb", borderRadius:8, padding:"10px 14px", marginBottom:12, fontSize:12, color:"#6b7280", display:"flex", alignItems:"center", gap:6 }}>
                  <HiOutlineCheckCircle size={13} color="#15803d" /> Vous avez déjà répondu à la demande de confirmation.
                </div>
              )}
              ...
              {!isClosed ? (
                <div style={{ border:"1px solid #d9d4cc", borderRadius:10, overflow:"hidden" }}>
                  <textarea rows={3} value={replyMsg} onChange={e => setReplyMsg(e.target.value)}
                    placeholder="Envoyer un message au technicien…"
                  ...
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"8px 10px", background:"#f9f6f2", borderTop:"1px solid #e8e2d9" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      <button type="button" onClick={() => fileInputRef.current?.click()}
                        ...
                        <HiOutlinePaperClip size={13}/> Joindre
                      </button>
                      {replyFiles.length > 0 && <span style={{ fontSize:11, color:"#94a3b8" }}>{replyFiles.length} fichier(s)</span>}
                    </div>
                    <button onClick={handleSendReply} disabled={sendingReply || (!replyMsg.trim() && replyFiles.length===0)}
                      ...
                      {sendingReply ? "Envoi…" : <><HiOutlinePaperAirplane size={13}/> Envoyer</>}
                    </button>
                  </div>
