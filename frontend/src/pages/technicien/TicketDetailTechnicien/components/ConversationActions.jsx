...
import { MessageSquare, CheckCircle2, Forward, Lock, Send, ShieldCheck, Clock, Info, AlertTriangle, Circle } from "lucide-react";
...
export default function ConversationActions({
  ...
  // States
  activeTab, respondMode, solution, infoMsg, solutionFiles, infoFiles, sending, sentError, clearSignal,
...
// Other
  isClosed, ticket, services, techniciens, solutionBlocked
}) {
      ...
      <div className="bg-white rounded-xl border border-gray-200 h-[500px]">
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
              ...
              <>
                ...
                <div>
                  <label className="text-[11px] font-semibold text-gray-500 mb-1.5 block">
                    {respondMode === "solution" ? "Solution" : `Message`}
                  </label>
                  <RichTextArea
                    onChange={respondMode === "solution" ? setSolution : setInfoMsg}
                    placeholder={respondMode === "solution" ? "Décrivez la solution..." : "Posez votre question..."}
                    rows={5}
                    disabled={solutionBlocked}
                ...
                <button onClick={handleSend}
                  disabled={sending || solutionBlocked || (!solution.replace(/<[^>]*>/g,"").trim() && !infoMsg.replace(/<[^>]*>/g,"").trim() && solutionFiles.length === 0 && infoFiles.length === 0)}
                  ...
                  <Send size={13}/>
                  {sending ? "Envoi..." : respondMode === "solution" ? "Envoyer solution" : "Envoyer"}
                </button>

                {respondMode === "solution" && (
                  <p className="text-[10px] text-gray-400 flex items-center gap-1">
                    <ShieldCheck size={11} className="text-green-500"/>
                    Confirmation automatique envoyée à l'employé.
                  </p>
                )}
              </>
            )
            ...
            isClosed ? (
              ...
              <>
                <div className="flex items-start gap-2 bg-purple-50 border border-purple-200 rounded-lg px-3 py-2 text-[11px] text-purple-700">
                  ...
                  Sélectionnez le technicien/service vers lequel rediriger.
                </div>
                <div>
          ...
          {activeTab === "close" && (
            isClosed ? (
              <div className="flex flex-col gap-3">
                <div className="flex items-center gap-2 text-gray-500 text-sm font-semibold"><Lock size={15}/> Fermé</div>
                {ticket.closing_note && (
                  <div className="bg-gray-50 border border-gray-200 rounded-xl p-3">
                    <p className="text-[10px] font-bold text-gray-500 uppercase mb-1">Note</p>
                    <p>{ticket.closing_note}</p>
                  </div>
                )}
              </div>
            ) : (
              <>
                {ticket?.is_resolved_confirmed && (
                  <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-2 text-[11px] text-green-700 flex items-center gap-2">
                    <CheckCircle2 size={13}/> Confirmé - prêt à fermer.
                  </div>
                )}
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 flex flex-col gap-3">
                  <div>
                    <p className="text-[12px] font-bold text-gray-800 mb-1">Déjà contacté ?</p>
                    <p className="text-[11px] text-gray-500">Fermez manuellement si résolu hors système.</p>
                  </div>
