...
const calculateSLA = () => {
  ...
  if (PAUSED.includes(tk.status)) {
    const frozen = tk.sla_pause_elapsed_ms != null ? Number(tk.sla_pause_elapsed_ms) : workingMsBetween(now, due);
...
{tk.assigned_at && <p style={{ fontSize: 11, color: "#a0a0a0", margin: "2px 0" }}>{t("ticketDetail.assignedOn")} : {formatDate(t, tk.assigned_at, "withTime")}</p>}
